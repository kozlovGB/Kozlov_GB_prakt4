CREATE DATABASE IF NOT EXISTS sem4; -- Создаете БД, если БД не создана
USE sem4;
CREATE TABLE `shops` (
	`id` INT,
...
VALUES 
 ...
 SELECT * FROM sem4.cats;
 SELECT * FROM sem4.shops;
-- Вывести всех котиков по магазинам по id (условие соединения shops.id = cats.shops_id)
SELECT c.name, s.shopname
FROM cats c -- cats = c
JOIN shops s -- INNER JOIN = JOIN, shops  = s
ON s.id = c.shops_id;
...
SELECT  s.shopname,cat_name.name
FROM shops s -- cats = c
JOIN (SELECT name, id,shops_id FROM cats WHERE name = "Murzik") AS cat_name
ON cat_name.shops_id = s.id;

-- Вывести магазины, в котором НЕ продаются коты “Мурзик” и “Zuza”
SELECT  s.shopname,cat_name.name
FROM shops s -- cats = c
JOIN (SELECT name, id,shops_id FROM cats WHERE NOT (name = "Murzik" OR  name = "Zuza")) AS cat_name
ON cat_name.shops_id = s.id;
